    ...
    async getSubjects(userId) {
        try {
            ...
            for (let course of subjects) {
                try {
                    const [avgWatchResult] = await database_1.default.execute(`SELECT COALESCE(ROUND(AVG(lp.watch_percent)), 0) as avg_watch_percent
             FROM lesson_progress lp
             WHERE lp.subject_id = ?`, [course.id]);
                    course.students_watch_percentage = avgWatchResult[0].avg_watch_percent || 0;
                }
                catch (err) {
                ...
                if (userId) {
                    try {
                        ...
                        const [completedResult] = await database_1.default.execute(`SELECT COUNT(*) as completed FROM lesson_progress 
               WHERE user_id = ? AND subject_id = ? AND is_completed = 1`, [userId, course.id]);
        ...
        try {
            ...
            if (userId) {
                ...
                for (let lesson of lessons) {
                    ...
                    try {
                        const [progressResult] = await database_1.default.execute(`SELECT is_completed, watch_percent FROM lesson_progress 
               WHERE user_id = ? AND lesson_id = ?`, [userId, lesson.id]);
                ...
                try {
                    const [completedResult] = await database_1.default.execute(`SELECT COUNT(*) as completed FROM lesson_progress 
             WHERE user_id = ? AND subject_id = ? AND is_completed = 1`, [userId, id]);
                    const completedCount = completedResult[0].completed;
